...
urlpatterns = [
    ...
    path('<slug:category_slug>/task<int:task_id>/solution<int:solution_id>/comment<int:comment_id>', CommentUpdateView.as_view(),
         name='edit_comment'),
    path('<slug:category_slug>/task<int:task_id>/solution<int:solution_id>/comment<int:comment_id>/delete', CommentDeleteView.as_view(),
         name='delete_comment')
]